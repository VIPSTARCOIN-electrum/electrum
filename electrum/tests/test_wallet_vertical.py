...
class TestWalletSending(TestCaseForTestnet):
    ...
    def _bump_fee_p2pkh_when_there_is_a_change_address(self, *, simulate_moving_txs):
        ...
        # create tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mQRag1cp5VavpANy4foiHMQEFhVS5RY5zd', 2500000)]
    ...
    def _bump_fee_p2wpkh_when_there_is_only_a_single_output_and_that_is_a_change_address(self, *, simulate_moving_txs):
        ...
        # create tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'tvips1q7rl9cxr85962ztnsze089zs8ycv52hk4ykevnl', '!')]
        coins = wallet.get_spendable_coins(domain=None)
        tx = wallet.make_unsigned_transaction(coins, outputs, fixed_fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325499
    ...
    def _bump_fee_when_user_sends_max(self, *, simulate_moving_txs):
        ...
        # create tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mQRag1cp5VavpANy4foiHMQEFhVS5RY5zd', '!')]
        coins = wallet.get_spendable_coins(domain=None)
        tx = wallet.make_unsigned_transaction(coins, outputs, fixed_fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325499
    ...
    def _rbf_batching(self, *, simulate_moving_txs):
        ...
        # create tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mQRag1cp5VavpANy4foiHMQEFhVS5RY5zd', 2_500_000)]
        coins = wallet.get_spendable_coins(domain=None)
        tx = wallet.make_unsigned_transaction(coins, outputs, fixed_fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325499
        ...
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'tvips1qy6xmdj96v5dzt3j08hgc05yk3kltqsnmm2td4g', 2_500_000)]
        coins = wallet.get_spendable_coins(domain=None)
        ...
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mbS4vRc3s4fxxYY2AE1u9NJWxYjJCxPgwL', 6_000_000)]
        coins = wallet.get_spendable_coins(domain=None)
...
class TestWalletOfflineSigning(TestCaseForTestnet):
    ...
    def test_sending_offline_old_electrum_seed_online_mpk(self, mock_write):
        ...
        # create unsigned tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'tvips1qyw3c0rvn6kk2c688y3dygvckn57525y84vwu83', 2500000)]
    ...
    def test_sending_offline_xprv_online_xpub_p2pkh(self, mock_write):
        ...
        # create unsigned tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'tvips1qp0mv2sxsyxxfj5gl0332f9uyez93su9cu4jfzj', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325340
        tx.version = 1
    ...
    def test_sending_offline_wif_online_addr_p2pkh(self, mock_write):  # compressed pubkey
        ...
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'tvips1quk7ahlhr3qmjndy0uvu9y9hxfesrtaht7zdlph', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325340
        tx.version = 1
    ...
    def test_sending_offline_hd_multisig_online_addr_p2sh(self, mock_write):
        ...
        # create unsigned tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mJ8XiwfbjLge5ivPZPZFU9m5GTfr6fuiJH', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325503
        tx.version = 1
    ...
    def test_sending_offline_hd_multisig_online_addr_p2wsh_p2sh(self, mock_write):
        ...
        # create unsigned tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mX91d6KBXWQrFSX4XMdvrVsggBowuN5Sg8', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325504
        tx.version = 1
    ...
    def test_sending_offline_hd_multisig_online_addr_p2wsh(self, mock_write):
        ...
        # create unsigned tx
        outputs = [TxOutput(bitcoin.TYPE_ADDRESS, 'mNjgpdVfxNHie4nXDpUogE37U8mfuQEPaw', 2500000)]
        tx = wallet_online.mktx(outputs=outputs, password=None, fee=5000)
        tx.set_rbf(True)
        tx.locktime = 1325505
        tx.version = 1
