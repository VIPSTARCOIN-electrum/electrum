...
;Variables
  ...
  !define PRODUCT_NAME "Electrum for VIPSTARCOIN"
  !define PRODUCT_WEB_SITE "https://github.com/VIPSTARCOIN-electrum/electrum-vips"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-vips-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-vips\electrum\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-vips\*.*"
  File "c:\electrum-vips\electrum\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-vips-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-vips-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-vips-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-vips-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-vips-${PRODUCT_VERSION}.exe" 0


  ;Links vipstarcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\vipstarcoin" "" "URL:vipstarcoin Protocol"
  WriteRegStr HKCU "Software\Classes\vipstarcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\vipstarcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\vipstarcoin\shell\open\command" "" "$\"$INSTDIR\electrum-vips-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\vipstarcoin"
